...
- **Timeline scope** (home "Recent", the archive, tags, RSS): the `COLLECTION_ORDER` collections plus labs, via `getPublicItems`/`getListItems`. Exception: the home "Recent" list excludes labs (`getRecentAcrossCollections` filters `kind: 'lab'`); labs still appear in the archive, tags, and RSS. Do **not** add `wiki`, `notes`, `inbox`, `specs`, or `idea` here — `wiki` is a category tree, not a dated timeline, and it carries no `/tags/` chips. Entry points: `posts`/`read-and-write` in the header nav; `wiki`/`notes`/`specs`/`inbox` in the footer only. `idea` is unlinked entirely (direct URL only, not in the footer).

...
Agentic Inbox는 Cloudflare Workers 위에서 동작하는 완전한 self-hosted 이메일 클라이언트로, 수신 이메일은 Cloudflare Email Routing으로 받고, 각 사서함은 독립 Durable Object 안에서 SQLite 데이터베이스로 격리되며, 첨부파일은 R2에 저장된다. 이 구조의 핵심 가치는 메일함마다 상태와 저장소가 분리되어 있어 한 사서함의 장애가 다른 사서함에 전파되지 않는다는 점이다.

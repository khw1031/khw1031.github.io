---
title: 회사 Swagger MCP Server 개발 및 도입
pubDate: '2025-12-18T14:08:02+09:00'
noteId: AGENT-2512-002
description: "회사 내부 Swagger 문서를 LLM이 자연어로 조회·연동하도록 drill-down 구조로 설계한 Model Context Protocol 서버의 개발·도입 기록"
summary: "OpenAPI 문서를 drill-down 방식(list_services→list_apis→get_api_detail→get_components)으로 LLM에 제공하는 Swagger MCP 서버 — API 연동 코드·타입·테스트 생성을 Agentic Coding 워크플로우에 통합해 컨텍스트 전환 비용을 제거한다."
lang: ko
tags: ['mcp', 'swagger', 'llm', 'api', 'agentic-coding']
lintHash: '20a095f8f3f5'
---

API 연동은 프론트엔드 개발에서 반복적이면서도 실수하기 쉬운 작업이다. Swagger 문서를 열어 endpoint를 찾고, request body 스키마를 확인하고, response 타입을 정의하는 일련의 과정이 매번 필요하다. Company Swagger MCP는 사용자가 등록한 회사 내부의 Swagger 문서를 탐색하여 이 과정을 LLM에게 위임할 수 있도록 설계된 Model Context Protocol 서버다.
...
```json
{
  "mcpServers": {
    "company-swagger-mcp": {
      "command": "npx",
      "args": ["-y", "@company/swagger-mcp@latest"],
      "env": {
        "SWAGGER_CONFIG_PATH": "/절대경로/swagger-config.json",
        "npm_config_@company:registry": "사내 registry 주소",

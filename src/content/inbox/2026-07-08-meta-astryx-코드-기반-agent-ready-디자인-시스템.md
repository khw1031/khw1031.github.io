...
lintHash: 'fb1145d0d012'
polishHash: 'fb1145d0d012'
---
...
"사람과 AI가 같은 참조를 읽는다"는 문구는 마케팅 슬로건이 아니라, 이 세 소비자가 전부 같은 `.doc.mjs`에서 파생된다는 실제 코드 구조를 가리킨다. 다만 이건 동시에 취약점이기도 하다 — 파생 원본이 하나라서, 그 원본이 틀리면 세 소비자가 함께 틀린다.

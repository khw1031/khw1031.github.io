...
lintHash: 'd6e57f143d6a'
---

## TL;DR
- AI 이미지 모델에 "스프라이트 시트"를 요청하면 생기는 캐릭터 불일치·배경 잔여물·격자 틀어짐을, **행(row) 단위 생성 → 크로마 알파 → 프레임 추출 → 아틀라스 + 매니페스트** 파이프라인으로 해결하는 오픈소스 Codex/Claude skill.
...
- sprite-gen은 이 문제를 한 번에 해결하려 하지 않고 **상태(state)별 한 행씩 생성**하는 component-row 방식으로 쪼개서, 각 행의 캐릭터 정체성을 기반 이미지로 고정한다.

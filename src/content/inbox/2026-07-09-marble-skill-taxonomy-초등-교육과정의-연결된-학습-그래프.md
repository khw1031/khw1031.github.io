...
pubDate: '2026-07-10T01:43:21+09:00'
noteId: LEARN-2607-019
...
lintHash: 'b80cc6f976e3'
polishHash: 'b80cc6f976e3'
---
...
## 핵심
- **왜 이 데이터셋이 필요한가?** ==대부분의 커리큘럼 데이터는 평면적인 표준 목록(flat list)이거나 특정 제품 안에 잠겨 있어, "A를 배우려면 B를 먼저 알아야 한다"는 학습의 연결 구조를 기계가 읽을 수 없었다.== Marble은 이를 열려 있는 **방향성 비순환 그래프(DAG)** 로 만들어 공개했다.
...
## 용어 풀이
- **micro-topic** — 한 번에 가르칠 수 있는 가장 작은 학습 단위.
- **prerequisite graph / DAG** — 선행학습 관계를 화살표로 연결한 그래프로, 순환(사이클)이 없음.
- **hard vs soft edge** — 필수 선행 vs 권장 선행.
- **ODbL 1.0 (Open Database License)** — 데이터베이스 구조·편성에 적용되는 라이선스.

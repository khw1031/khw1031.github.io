...
lintHash: 'b6294835ff7f'
polishHash: 'b6294835ff7f'
---
...
- 저자의 핵심 명제: **"에이전트는 당신이 리뷰할 수 있는 것보다 더 많이 출하할 수 있다."** → 따라서 scarce resource는 생성이 아니라 **판단(judgment)**이다.
...
- **(보강)** Alpha/Decay는 저자가 새로 만든 말이 아니라 **퀀트 트레이딩 용어를 커리어 경제학에 그대로 옮겨온 것**이다 — 원래 alpha는 시장 평균(베타)을 넘어서는 초과 수익, alpha decay는 그 전략이 알려지고 남들도 따라 하면서 초과 수익이 사라지는 현상을 가리킨다(Grinold & Kahn류 액티브 운용 이론의 표준 개념). 저자는 이 구도를 그대로 "에이전트가 실행을 맡으면 코딩 실력 같은 Alpha도 결국 Decay(평준화)되고, 남는 희소 자원은 아직 지표화되지 않은 다음 Alpha를 먼저 감지하는 Taste"라는 주장으로 확장한다.
- **(교차 참조)** 이 Alpha/Decay/Taste는 다른 노트(`2026-07-11-llm-에이전트가-촉발한-인지적-망상과-전문성의-역할.md`)의 "전문성 + 비판적 사고" 축과 같은 질문("AI 시대에 전문성이 왜 여전히 가치 있는가")에 대한 더 공격적인 답이다 — 그 노트의 전문성이 "이미 나온 AI 답을 검증하는" 수동적 역할이라면, Taste는 "AI가 아직 흡수하지 못한 엣지를 먼저 알아채는" 능동적 역할.
...
- **Outer Loop** — 시스템을 둘러싼 인간의 검증·판단·책임 사이클.

- **Inner Loop** — 에이전트가 investigation→implementation→verification→repeat를 반복하는 실행 사이클.

- **Back Pressure** — 파이프라인의 흐름을 늦추거나 제어하는 역방향 압력.

- **Cognitive Surrender** — AI 결과를 비판 없이 받아들이는 인지적 태만.

- **Answerability** — "왜 이렇게 했는가?"에 대해 증거 기반으로 설명할 수 있는 보증.

- **Taste** — 객관적 지표가 없는 상황에서 변화를 감지하는 질적 판단력.

- **Alpha** — 최고 성과자가 취하는 선두 포지션.
- **Decay (alpha decay)** — 반복과 관찰로 모든 사람이 배우게 되는 확립된 패턴(평준화)

...
- **Alpha** — 최고 성과자가 취하는 선두 포지션 / 비유: 퀀트 트레이딩에서 시장 평균(베타)을 넘어서는 **초과 수익** / 비유가 깨지는 지점: 금융에서 alpha는 숫자로 측정 가능하지만, 커리어에서의 alpha는 아직 지표화되지 않은 경우가 많아 측정 자체가 taste의 영역이다.
- **Decay (alpha decay)** — 반복과 관찰로 모든 사람이 배우게 되는 확립된 패턴(평준화) / 비유: 트레이딩 전략이 알려지고 남들도 따라 하면서 초과 수익이 사라지는 현상 / 비유가 깨지는 지점: 금융시장의 alpha decay는 경쟁자의 모방으로 생기지만, 여기서는 AI가 그 패턴을 학습·재현하면서 생긴다는 점이 다르다.

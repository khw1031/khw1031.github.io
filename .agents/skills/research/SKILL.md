...
3. **Placement gate** — 작성 전에 반드시 `src/content/wiki/` 트리를 스캔한다:
   - 같은 카테고리가 있으면 그 아래 leaf로 추가하고 카테고리 `index.md`의 맵을 갱신한다.
   - 인접 카테고리가 있으면 본문에서 인라인 링크로 상호 참조한다.
...
OKF 개념 그래프(형제 레퍼런스·다른 카테고리·notes로의 링크)는 별도 §연결 섹션이 아니라
§핵심 산문과 §레퍼런스 안의 인라인 마크다운 링크로 표현한다 — §연결·§곁가지 섹션은 쓰지 않는다.
...
- notes(unlisted 개인 학습, core)와 구분한다. 개인 학습 노트는 core로, 공개 레퍼런스
  라이브러리는 research로 간다.

...
| [weave](weave/SKILL.md) | Self-driven learning runner (sibling to global `dig`, opposite mode). User implements themselves; questions on 막힘/궁금 go to an append-only staging log (`~/.weave/`), then migrate into `src/content/notes` as categorized layered notes with 앞/뒤 links (cytoscape graph material). Entry difficulty gate; bug-vs-gap split; 3 hidden-answer 확인 질문 as minimum retrieval gate. |

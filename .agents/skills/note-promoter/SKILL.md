...
4. **Map first** — 한 줄 명제 + MECE 가지의 ASCII 맵을 세부보다 먼저 만든다. 가지 이름이
   곁가지 노트 후보이자 나중 인출 cue가 된다. 이 맵은 관계가 한 종류(포함)라 ASCII로
   충분하다 — 반면 §핵심·§깊이에서 관계가 여러 종류인 구조 다이어그램(의존 vs 구현,
   import vs 호출)을 그릴 때는 ```mermaid 코드블록으로 선 종류를 구분한다(빌드타임 SVG
   렌더). 화살표 한 스타일에 두 관계를 섞으면 방향 오개념을 만든다 — AGENTS.md §Diagrams.

...
description: >
  ...
  (notes, inbox, sources, idea) by first mapping every inbound reference to it across
  ...
  children. Use when the user asks to delete/remove a note, idea note, spec, or inbox
  capture, or to clean up an obsolete doc without leaving dangling links. 노트·아이디어·
...
repo-operating-targets: src/content/notes, src/content/inbox, src/content/sources, src/content/idea
argument-hint: "[삭제할 문서 경로|slug (notes|inbox|sources|idea)]"
---
...
- **Deletable targets**: any doc under `src/content/{notes,inbox,sources,idea}`
...
- **Reference scan is repo-wide**: an inbound link to the target can live in any
  collection (a post linking a note, an idea note linking a note), or in
  `AGENTS.md`/`CLAUDE.md`, skills, or config. Scan all of it (read-only).

## When to use

- "이 노트/아이디어/스펙 삭제해줘" — a single doc, safely.
...
- Cleaning up an obsolete/duplicated capture without leaving `/notes/...` or
  `/idea/...` links pointing at nothing.
...
2. **Reference scan (repo-wide, read-only).** Find inbound references and group them
   by strength:
   - **URL links** — `/{collection}/{slug}/` (e.g. `/notes/foo/`, `/idea/bar/`),
...
- Listing/search/sitemap/routing code. notes/inbox/sources/idea are all unlisted;
  pagefind/sitemap regenerate on the next build — no manual index surgery. Changing a collection's exposure is escalation, not deletion.
...
- `src/content/{notes,inbox,sources,idea}`의 파일을 삭제하고, 승인된 inbound 링크만 편집한다.

...
- **Never moves files across collections.** Polish highlights and aligns structure
  *within* a collection. Moving a capture *out* of inbox (inbox→notes) is authoring,
  not this pass — polish only runs on the note that already landed.
...
- **notes** → the topic-tree model: a flat note that has grown ≥2
...
- **관점 대필**: idea의 `§열린 질문` 등 사용자 목소리 서술을 rewrite하는 것. HARD FAIL —
  하이라이트는 감쌀 수 있으나 문장을 바꾸지 않는다.
- **폐지 섹션 부활**: §비유·§곁가지·§연결·§인출 질문·§내 관점을 구조 정렬 과정에서

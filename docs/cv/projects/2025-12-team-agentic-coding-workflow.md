---
project_id: '2025-12-team-agentic-coding-workflow'
title: '팀 기반 Agentic Coding Workflow 구축'
period: '2025.12'
company: '(주)한샘'
role: 'AI 워크플로우 설계 / 표준화'
status: needs-input
source: 'src/content/inbox/2025-12-10-team-agentic-coding-workflow.md'
post: 'src/content/inbox/2025-12-10-team-agentic-coding-workflow.md'
public_url: 'https://khw1031.github.io/posts/20251210/'
evidence_dir: 'docs/cv/evidence/2025-12-team-agentic-coding-workflow/'
cv_ready: false
post_ready: false
public_references:
  - 'https://khw1031.github.io/posts/20251210/'
  - 'https://github.com/khw1031/pilot'
private_materials: []
---

# 팀 기반 Agentic Coding Workflow 구축

## CV Summary

작성 전. 바이브 파일럿, Feature Workflow Skill, AI Toolkit과 경계가 겹치므로 독립 프로젝트로 둘지 상위 개념으로 묶을지 결정 필요.

## Post Summary

팀 단위 에이전틱 코딩을 개인 프롬프트가 아니라 Grounding Context, Human Checkpoint, 표준화된 명령어 워크플로우로 통제해야 한다는 전략 글.

## Linked Documents

- [Inbox draft](../../../src/content/inbox/2025-12-10-team-agentic-coding-workflow.md)
- [Metrics](../evidence/2025-12-team-agentic-coding-workflow/metrics.md)
- [Interview Questions](../evidence/2025-12-team-agentic-coding-workflow/interview-questions.md)
- [Concepts](../evidence/2025-12-team-agentic-coding-workflow/concepts.md)
- [References](../evidence/2025-12-team-agentic-coding-workflow/references.md)
- [Rewrite Notes](../evidence/2025-12-team-agentic-coding-workflow/rewrite-notes.md)

## Missing Information

- [ ] 파일럿 레포지토리와 사내 적용 사례의 경계
- [ ] 실제 팀 적용 범위와 채택률
- [ ] 표준 워크플로우 도입 전후 비교 수치
- [ ] CV에는 독립 프로젝트로 둘지 다른 AI Workflow 프로젝트의 근거 문서로 둘지 결정

## Deep-Dive Topics

- Agentic Coding과 Vibe Coding의 차이
- Human in the Loop 설계
- Prompt Drift와 Cascading
- `.ai` 디렉터리 기반 Grounding Context
- 컨텍스트 분리와 작업 문서 체계

## Draft CV Paragraph

작성 전.
